...
def test_load_or_create_fppool_atom_fps_round_trips_cache(tmp_path):
    ...
    config = FPPoolConfig(morgan_nbits=32, morgan_radius=2)
    ...
    assert family_names_1 == ["morgan"]
    assert np.array_equal(family_lengths_1, np.array([32]))
    ...
    for atom_fp_1, atom_fp_2, mol in zip(atom_fps_1, atom_fps_2, mols):
        ...
        assert atom_fp_1.shape == (mol.GetNumAtoms(), 32)
